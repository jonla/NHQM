...
import bases.momentum
from problems import Helium5
from helpers.quantum import absq
from helpers.plotting import find_resonance_state
...
y_peak=-0.07
quantum_numbers = quantum_numbers.QuantumNumbers(l, j)

'''
Triangular basis, so the contour contains resonance states. Is mom_tri a berggren-basis now?
'''
contour_tri = bases.momentum.triangle_contour(x_peak, y_peak, k_max, basis_state_count, 5)
mom_tri = bases.momentum.MomentumBasis(contour_tri) 
...
'''
contour_str = bases.momentum.gauss_contour([0, k_max], basis_state_count)
mom_str = bases.momentum.MomentumBasis(contour_str)
...
'''
states_tri=solve.solve(problem, quantum_numbers, mom_tri)
states_str=solve.solve(problem, quantum_numbers, mom_str)
